...
def move():
    ...
    if form.validate_on_submit() and request.method == 'POST' :
        ...
        if boolbeans == False:
            ...
            flash(f'No products in this loaction .Please add products', 'danger')
        else:
...
def check(frm,to,name,qty):
    if frm == to :
        ...
        a=str(bal)
        if(a=='None'):
            flash(f'No products in source location.Please add products', 'danger')

        else:
            bal = Balance.query.filter_by(location=to,product=name).first()
            a = str(bal)
            if a=='None' and  bal.quantity >= qty:
                new = Balance(product=name,location=to,quantity=qty)
                db.session.add(new)
                bal.quantity += qty
                ...
                a = str(bal)
                if(a=='None'):
                    new = Balance(product=name,location=frm,quantity=qty)
                    db.session.add(new)
                else:
